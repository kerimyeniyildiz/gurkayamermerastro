        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-12 border-t border-emerald-800/50 pt-12">
          {features.map((feature, idx) => (
            <div key={idx} className="group">
              <div className="mb-6 text-amber-500 group-hover:text-amber-400 transition-colors duration-300">
                {feature.icon}
              </div>
              <h3 className="font-serif text-xl mb-3 text-white tracking-wide">{feature.title}</h3>
              <p className="text-emerald-200/60 text-sm leading-relaxed font-light">
                {feature.description}
              </p>
            </div>
          ))}
