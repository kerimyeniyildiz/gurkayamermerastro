...
export const ProductShowcase: React.FC = () => {
  const [activeBrand, setActiveBrand] = useState<'Porselen' | 'Belenco' | 'Çimstone'>('Porselen');
  ...
  return (
    <section id="products" className="py-24 bg-white scroll-mt-20">
      <div className="container mx-auto px-6">
        ...
        {/* Elegant Tabs - Updated layout to prevent scroll artifacts */}
        <div className="flex justify-center mb-12 border-b border-stone-100">
          <div className="flex flex-wrap justify-center gap-x-8 md:gap-x-16 gap-y-4 pb-4">
            {(['Porselen', 'Belenco', 'Çimstone'] as const).map((brand) => (
              <button
                key={brand}
              ...
              >
                {brand}
                {activeBrand === brand && (
                  <span className="absolute -bottom-[17px] left-0 w-full h-1 bg-amber-700 rounded-t-full"></span>
                )}
              </button>
            ))}
          </div>
          ...
          {filteredProducts.map((product) => (
            <div key={product.id} className="group">
              ...
              <div className="flex justify-between items-baseline border-b border-stone-100 pb-4">
                <div>
                   <h3 className="font-serif text-2xl text-emerald-950 mb-1 group-hover:text-amber-700 transition-colors cursor-pointer">
                     {product.name}
                   </h3>
